...
**Последнее обновление:** 2026-05-14, после Фазы 0.3.
...
- **Фаза:** 0 (Фундамент). Сделано 0.1, 0.1a, 0.2, 0.3. **Следующее — 0.4.**
...
**Фаза 0.4 — API-клиент (`src/api/`).** Спека для сабагента: типизи-
рованный клиент поверх Zulip REST API на доменных типах из
`src/domain`. Покрыть как минимум: auth (`fetch_api_key`), register
event queue + long-poll `GET /events`, fetch messages (`GET /messages`
с narrow), send message, реакции, подписки, users/streams. Сеть —
один общий клиент (не голый `fetch` по компонентам). Базовый URL —
через Vite dev-proxy `/api` → стенд (PRD §7.4). Источник истины по
эндпоинтам — `zerver/openapi/zulip.yaml` в корне репозитория. Realtime
register/long-poll механизм клиент предоставляет, но подписки
(`src/realtime/`) — отдельная Фаза 1.2. Сабагент обязан прочитать
GUIDE + REGISTRY первой строкой спеки.

После 0.4: 0.5 app-shell → 0.6 примитивы (параллельно).
