...
**Последнее обновление:** 2026-05-15, после Фазы 1.5.
...
- **Фаза:** Фаза 0 завершена. Фаза 1 — сделаны 1.1–1.5.
  **Следующее — 1.5a (доборка данных сайдбара), затем 1.6.**
...
- [x] 1.5 Левый сайдбар — `src/features/leftSidebar/` (виды/ЛС/каналы
      ...
      реструктурирован в бакетированный; 433 unit-теста; гейты зелёные.
      Протык: структура + skeleton-loading + интеграция в shell — ✅;
      populated-состояние — ⏳ с владельцем. Пробелы → 1.5a (ниже).

---

## Следующее действие

**Фаза 1.5a — доборка данных сайдбара** (пробелы, вскрытые 1.5).
Сделать до 1.6, т.к. лента тоже опирается на топики:
- **Иконки видов** — добавить в `src/icons/` глифы для 7 built-in-видов
  (inbox/recent/combined-feed/mentions/reactions/starred/drafts) и
  подключить в `ViewsSection`. (`reactions` ≈ существующий `smile`.)
- **`dmConversationsStore`** — полный список ЛС-бесед (не только с
  непрочитанным), recency-сортировка. Сейчас секция ЛС показывает
  только беседы с unread (из `unreadStore.dms`).
- **`topicsStore` + `getTopics` в `apiClient`** — полный список топиков
  канала. Сейчас под каналом видны только топики с непрочитанным.
  Понадобится и ленте 1.6 (recipient-бары).
- **`mentions`-бакет в `unreadStore`** — снапшот даёт `unread_msgs.
  mentions`, сейчас отбрасывается; нужен для бейджей видов
  Mentions/Starred.

После 1.5a: 1.6 лента сообщений (виртуализация, recipient-бары,
группировка, дозагрузка — пишет в `messagesStore.ingest`), 1.7 рендер
контента (гидрация `rendered_content`), 1.8 правый сайдбар. 1.6 —
крупный и центральный. Детали — PRD §8 ФАЗА 1, чеклист §10.
