...
**Последнее обновление:** 2026-05-15, после Фазы 1.6.
...
- **Фаза:** Фаза 0 завершена. Фаза 1 — сделаны 1.1–1.6.
  **Следующее — 1.7 (рендер контента сообщений).**
...
**Фаза 1.7 — рендер контента сообщений (`src/features/`).** Гидрация
серверного `rendered_content` (HTML) сообщения: санитизация (XSS!),
подсветка кода, KaTeX, эмодзи, спойлеры, упоминания, ссылки.
**Seam уже есть** — `MessageContent` в `src/features/messageFeed/`
сейчас рендерит plain-text; 1.7 заменяет его тело на санитайзер +
дерево элементов, не трогая имя/пропсы (раскладка строки не меняется).
Санитизация — обязательна; рассмотреть либу (DOMPurify и т.п.) — через
оркестратора. Гидрация — unit-тесты обязательны.

После 1.7: 1.8 правый сайдбар → **гейт Фазы 1** (Playwright login→
narrow→лента + общий протык). Детали — PRD §8 ФАЗА 1, чеклист §10.
Сабагенты: GUIDE+REGISTRY первой строкой, не коммитят.
...
- 1.6: **у `apiClient` нет таймаута запроса** — вскрыто протыком: при
  зависшем/недоступном стенде `getMessages`/`register` висят pending
  бесконечно, лента застревает в loading без error-пути. Нужен
  AbortController-таймаут в `src/api/request.ts` — небольшой
  изолированный фикс, **сделать в начале 1.7 или отдельным проходом.**
...
- 1.6: recipient-бары — in-flow виртуализированные строки, не
  ...
  доработка (overlay-заголовок). `MessageContent` рендерит plain-text
  до 1.7.
