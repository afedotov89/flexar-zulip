...
**Последнее обновление:** 2026-05-15, после Фазы 1.2.
...
- **Фаза:** Фаза 0 завершена. Фаза 1 — сделаны 1.1, 1.2.
  **Следующее — 1.3 (сторы server-state).**
...
**Фаза 1.3 — сторы server-state (`src/stores/`).** Сторы под realm,
users, streams/subscriptions, messages, presence, unread. Паттерн —
как `authStore` (Zustand, см. REGISTRY «Сторы»). Подписываются на
`realtimeConnection.subscribe(...)` и редуцируют `ServerEvent` в стейт;
редьюсеры — чистые, обязательно unit-тесты.

**РЕШИТЬ ДО СТАРТА 1.3** (флаги от 1.2):
1. **Initial state.** `register` отдаёт начальное состояние, но 1.2
   его НЕ потребляет. Варианты: (а) сторы тянут initial через REST
   (`getSubscriptions`/`getUsers`/…); (б) расширить `RealtimeConnection`,
   чтобы отдавал register-payload, и сторы гидрируются из него.
   Вариант (б) ближе к тому, как работает Zulip (одним `register`).
   → выбрать при запуске спеки 1.3.
2. **Нет буфера событий** — сторы должны подписаться на `module-load`,
   до `start()`. Заложить в спеку.

После 1.3: 1.4 роутинг/narrow → 1.5–1.8 (сайдбары, лента, рендер
контента). Детали — PRD §8 ФАЗА 1, чеклист §10. Сабагенты: читают
GUIDE+REGISTRY первой строкой, переиспользуют примитивы/стор-паттерн,
не коммитят.
