...
**Последнее обновление:** 2026-05-15, после Фазы 1.3.
...
- **Фаза:** Фаза 0 завершена. Фаза 1 — сделаны 1.1, 1.2, 1.3.
  **Следующее — 1.4 (роутинг/narrow).**
...
**Фаза 1.4 — роутинг/narrow (`src/lib/` + роуты).** URL ↔ narrow:
парсер narrow (строка операторов ↔ `Narrow` из `src/domain`), мэппинг
встроенных видов (Входящие/Последние/Объединённая лента/Упоминания/
Реакции/Отмеченные/Черновики) и channel/topic/DM/search на URL и
обратно. Роуты внутри `AppShell` под конкретные narrow. Парсер —
чистый, обязательно unit-тесты. Сторы 1.3 уже дают server-state;
лента (1.6) будет читать narrow отсюда.

После 1.4: 1.5 левый сайдбар · 1.6 лента сообщений · 1.7 рендер
контента · 1.8 правый сайдбар (часть можно параллелить). Детали —
PRD §8 ФАЗА 1, чеклист §10. Сабагенты: читают GUIDE+REGISTRY первой
строкой, переиспользуют примитивы/стор-паттерн, не коммитят.
