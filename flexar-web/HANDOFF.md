...
**Последнее обновление:** 2026-05-15, после Фазы 1.7.
...
- **Фаза:** Фаза 0 завершена. Фаза 1 — сделаны 1.1–1.7.
  **Следующее — 1.8 (правый сайдбар), затем гейт Фазы 1.**
...
**Фаза 1.8 — правый сайдбар (`src/features/`).** Правая колонка
`AppShell`. Список пользователей с presence (`usersStore` +
`presenceStore`), секции «этот канал» / «эта беседа» (контекст из
текущего narrow — `useCurrentNarrow`), фильтр пользователей. Данные —
из сторов 1.3; примитивы — из реестра (`Avatar`, presence-индикатор
есть в leftSidebar — переиспользовать/вынести при необходимости —
флагнуть, если нужен общий). Доменный блок → `src/features/`,
монтируется в правый `<aside>` `AppShell`.

После 1.8: **гейт Фазы 1** (Playwright login→narrow→лента + общий
протык; `e2e/smoke.spec.ts` устарел — обновить под login-flow).
Детали — PRD §8 ФАЗА 1, чеклист §10. Сабагенты: GUIDE+REGISTRY первой
строкой, не коммитят.
