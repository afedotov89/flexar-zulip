...
**Следующее — Фаза 2 (путь записи), PRD §8 ФАЗА 2:**
- ✅ **2.1+2.2** — Compose-бокс + send. **Live: сообщение отправлено.**
- ✅ **2.3 Typeahead** — `@юзеры`/`#каналы`/`:эмодзи` (140 bundled) +
  топики; bespoke listbox с `aria-activedescendant`. **Live: все три
  показали правильные опции на стенде.** 749 unit-тестов.
- **2.4 Черновики** ← следующее. Автосохранение, список, восстановление.

(Реакции / редактирование / поиск — это Фаза 3 по PRD.)
