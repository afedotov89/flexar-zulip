...
**Последнее обновление:** 2026-05-15, после Фазы 1.5a.
...
- **Фаза:** Фаза 0 завершена. Фаза 1 — сделаны 1.1–1.5 + 1.5a.
  **Следующее — 1.6 (лента сообщений).**
...
**Фаза 1.6 — лента сообщений (`src/features/`).** Центральная колонка
`AppShell`. Виртуализированный список сообщений; recipient-бары
(канал>топик / ЛС); дата-разделители; строка сообщения (аватар,
отправитель, время, контент, hover-контролы); группировка
последовательных сообщений одного отправителя; состояния loading/empty;
дозагрузка старых/новых по скроллу. Данные: читает текущий narrow через
`useCurrentNarrow`; тянет историю через `apiClient.getMessages` и пишет
в `messagesStore.ingest`; живые обновления — уже в `messagesStore` из
событий. Топики/каналы/юзеры — из сторов 1.3/1.5a. **Рендер контента
сообщения (гидрация `rendered_content`) — отдельная Фаза 1.7;** в 1.6
контент можно временно выводить как есть/плейсхолдером, чтобы не
блокироваться. Виртуализация — без новых зависимостей по умолчанию;
если нужен либ-вирт (`@tanstack/virtual` и т.п.) — флагнуть оркестратора.

После 1.6: 1.7 рендер контента, 1.8 правый сайдбар. Детали — PRD §8
ФАЗА 1, чеклист §10. Сабагенты: GUIDE+REGISTRY первой строкой,
переиспользуют примитивы/сторы/narrow-хуки, не коммитят.
