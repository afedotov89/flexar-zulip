...
- **Прод:** <https://<host>:8843/> (self-signed cert, realm "Friflex")
...
```
   ...
   ────────────────────────►│  • docker buildx (native arm64)     │
                            ...
                            └────────────────┬────────────────────┘
                                             ...
                                             ▼
                     ┌────────────────────────────────────────────┐
                     │  <deploy-host> (<deploy-host>, ARM64, Debian 12)   │
                     │  <host>  (user: deploy, uid 1001)    │
                     │                                            │
...
Хост — <deploy-host> на 16 ГБ RAM, 29 ГБ диска. **Другие проекты тоже там
...
# 6. Проверь:
curl -k https://<host>:8843/api/v1/server_settings | jq .zulip_version
...
```bash
ssh user@<host>           # обычный user, passwordless sudo
ssh -i ~/.ssh/flexar_zulip_deploy deploy@<host>
...
8. **`SETTING_FAKE_EMAIL_DOMAIN` обязателен для IP-only deploy.** Zulip
   ...
   Если EXTERNAL_HOST — IP (`<host>`), это не valid email → home page
...
- **Self-hosted в одном инстансе на <deploy-host>** — это dev/staging, не
